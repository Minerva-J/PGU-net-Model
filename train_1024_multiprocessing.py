...
parser = argparse.ArgumentParser(description='使用 torch.multiprocessing 取代启动器')
def main():
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = torch.utils.data.DataLoader(train_dset, batch_size=batch_size, sampler=train_sampler)
    #######################################
    val_sampler = torch.utils.data.distributed.DistributedSampler(val_dset)
    val_loader = torch.utils.data.DataLoader(val_dset, batch_size=batch_size, sampler=val_sampler)
    #######################################
    test_sampler = torch.utils.data.distributed.DistributedSampler(test_dset)
    test_loader = torch.utils.data.DataLoader(test_dset, batch_size=batch_size, sampler=test_sampler)
    #######################################
